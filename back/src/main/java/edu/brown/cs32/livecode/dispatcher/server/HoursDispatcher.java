...
public class HoursDispatcher {
    ...
    HoursDispatcher(HelpRequesterQueue signups, DebuggingPartnerQueue debuggingPartnerQueue, String statusMessage) {
        this.queue = signups;
    ...
    public void dispatch(SessionState sessionState)  {
        ...
        while(sessionState.getRunning()) {
            if(queue.getNeedHelp().hasNext()) {
                ...
                // Help the student
                System.out.println(Utils.timestamp()+" Dispatcher: Hi "+ nextHelpRequester +"; you'll be seen by "+helper);
                try {
        ...
        System.out.println(Utils.timestamp()
            + " Dispatcher: The current session has ended.");
    }
